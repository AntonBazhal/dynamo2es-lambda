...
const lambdaHandler = require('@alpha-lambda/handler');
...
module.exports = function(options) {
  ...
  const esclient = elasticsearchOpts.client || aeclient.create(_.omit(elasticsearchOpts, 'bulk'));
  ...
  const addBunyan = _.has(options, 'bunyan') ? options.bunyan : true;
        ...
        const parsedEvent = await event.Records.reduce(async (acc, record) => {
          try {
            ...
            if (options.transformRecordHook) {
              doc = await options.transformRecordHook(doc, parsedRecord.OldImage, context);
            }
