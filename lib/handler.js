...
module.exports = function(options) {
  ...
  const elasticsearchOpts = options.elasticsearch || {};
  const bulkOpts = elasticsearchOpts.bulk || {};
  const esclient = elasticsearchOpts.client;

  const separator = has(options, 'separator') ? options.separator : '.';
      ...
      return Promise.resolve()
        .then(() => {
            ...
            try {
              ...
              const idResolver = options.idResolver || (() => {
                return options.idField
                  ...
                  : utils.assembleField(parsedRecord, keys(parsedRecord.Keys), separator);
              });
              ...
              if (doc) {
                ...
                switch (record.eventName) {
                  ...
                  case 'REMOVE':
                    if (has(actionDescriptionObj, 'version')) {
          ...
          return promiseRetry(retry => {
            return esclient
              .bulk(defaults({ body: parsedEvent.actions }, bulkOpts))
              .then(result => {
                return {
                  result,
                  meta: parsedEvent.meta
